...
def _publisher_failure(exc: BaseException) -> tuple:
    """F2: transport_down only for timeout / client / OS errors.

    Any other exception refuses only the symbol that raised it. A timeout is
    always the named reason PUBLISHER_TIMEOUT, including when the public
    client wraps it after its own retry.
    """
    import asyncio
    try:
        import aiohttp
        client_error = aiohttp.ClientError
    except ImportError:  # pragma: no cover - aiohttp is a locked pin
        client_error = ()
    chain = []
    ...
    timed_out = any(isinstance(item, (asyncio.TimeoutError, TimeoutError))
                    for item in chain) or any(
                        "TimeoutError" in type(item).__name__ or "TimeoutError" in str(item)
                        for item in chain)
    if timed_out:
        return "PUBLISHER_TIMEOUT", True
    if any(isinstance(item, client_error) for item in chain):
        return "PUBLISHER_TRANSPORT_UNAVAILABLE", True
    if any(isinstance(item, OSError) for item in chain):
        return "PUBLISHER_TRANSPORT_UNAVAILABLE", True
    return str(getattr(exc, "reason", None) or type(exc).__name__), False
...
class PaperRuntime:
    ...
    async def _refresh_publishers(self, due: Sequence) -> Dict[str, Any]:
        ...
        transport_down is set only for a timeout, an aiohttp client error, or
        an OSError. Any other exception refuses that symbol by a named reason
        ...
        for symbol in symbols:
            ...
            try:
                ...
                reason, down = _publisher_failure(exc)
                report["failures"].append({
                    "symbol": symbol, "publisher": "funding",
                    "reason": reason})
                self.refusals.append({"cell": symbol, "reason": reason})
                if down:
                    transport_down = True
            try:
                latest = await self.store.db.execute(
    ...
    async def run_cycle(self, *, now_ms: Optional[int] = None) -> Dict[str, Any]:
        ...
        published = (cycle.get("catch_up") or {}).get("quality_publish") or {}
        cycle["publishers"]["quality_facts_written"] = int(
            cycle["publishers"].get("quality_facts_written") or 0) + int(
                published.get("written") or 0)
        cycle["publishers"]["failures"] = list(
            cycle["publishers"].get("failures") or [])
        cycle["publishers"]["failures"].extend(list(published.get("failures") or []))
