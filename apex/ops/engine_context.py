...
def paper_package_binding(*, environment: str, params_dir: str | Path | None = None) -> dict | None:
    ...
    # D59 د۳ / F3: bootstrap has no calibration history. The package is named
    # uncalibrated. It does not invent rolling_calibration_error/brier/log_loss
    # zeros, and those keys are not part of the YAML digest.
...
class _TimeoutVisibleSession:
    """The public client wraps TimeoutError without chaining it, and
    ``str(TimeoutError())`` is empty. Mark the timeout so a caller can still
    name it. The request timeout itself stays the client's 10s value.
    """
    ...
    def get(self, *args: Any, **kwargs: Any) -> Any:
        return _TimeoutVisibleCall(self._session.get(*args, **kwargs))

    def __getattr__(self, name: str) -> Any:
        return getattr(self._session, name)


class _TimeoutVisibleCall:
    ...
    async def __aenter__(self) -> Any:
        try:
            ...
            raise TimeoutError("TimeoutError") from exc

    async def __aexit__(self, exc_type: Any, exc: Any, tb: Any) -> Any:
        return await self._inner.__aexit__(exc_type, exc, tb)


def _reraise_publisher_timeout(exc: BaseException) -> None:
    chain = []
    cause: BaseException | None = exc
    while cause is not None and cause not in chain:
        chain.append(cause)
        cause = cause.__cause__
    timed_out = any(isinstance(item, TimeoutError) or "TimeoutError" in str(item)
                    or "TimeoutError" in type(item).__name__ for item in chain)
    if timed_out:
        raise TimeoutError("PUBLISHER_TIMEOUT") from exc
...
async def collect_public_venue_facts(symbol: str, *, client: Any = None,
    ...
    if client is None:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
            client = ToobitPublicClient(session=_TimeoutVisibleSession(session))
            try:
                return await collect_public_venue_facts(symbol, client=client, now=now)
            except Exception as exc:
                _reraise_publisher_timeout(exc)
                raise
...
async def collect_public_funding_schedule(symbol: str, *, client: Any = None,
    ...
    if client is None:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
            client = ToobitPublicClient(session=_TimeoutVisibleSession(session))
            try:
                return await collect_public_funding_schedule(symbol, client=client, now=now)
            except Exception as exc:
                _reraise_publisher_timeout(exc)
                raise
    from apex.execution.toobit_map import to_wire_symbol
