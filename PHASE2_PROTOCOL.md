# APEX_GEN5 — PHASE 2 COMMON PROTOCOL (LAW FOR EVERY AGENT)

Applies to: AGENT-01 … AGENT-10, equally, without exception. This file preserves and re-bases the binding sections of `PROMPT.md` (Chief Engineer directive) for a multi-agent chain. Read this file IN FULL before doing anything else. It is short on purpose.

Provenance map (PROMPT.md section → here): §1→P1, §2/§7→P2, §3→P3, §4→P4, §5→P5, §6→P6, §8→P9, §9→P10, §10→(per-agent scope), §11→P8, §12→P11, §13→P13, §14→P14, §15→P11, §16→P17, §17→P18, §18→P16/P18, §19→PHASE2_CHECKPOINTS exit gates.
...
- Frozen specification: `APEX_GEN5.md` (repo root). It is the single authoritative source of implementation truth. Before writing any code you MUST read, in full, the blueprint ranges assigned to you in your prompt file, and build your internal implementation map for that range. No coding begins before your assigned specification is understood.
...
An agent that silently resolves a contradiction is in breach of P4 exactly like one that invents a feature. Equally: an agent that BLOCKS on a question the pre-adjudications (ADR-P2-001..014) already answer is in breach of efficiency law P15 — read the DECISION_LOG before your first line of code.
...
- READ IN FULL, in order: (1) your prompt file; (2) this PROTOCOL; (3) `PHASE2_CHECKPOINTS.md` (your CP section only); (4) `PHASE2_DECISION_LOG.md` (rules + open-issues + ADR table); (5) the handoff file(s) of your predecessor CP(s) named in your prompt — they are capped at 400 lines per slot by their authors precisely so this read is cheap; (6) YOUR assigned blueprint ranges from `APEX_GEN5.md` (use the line ranges; read them in 400–800 line slices via file tools, never re-read twice — extract notes as you go).
