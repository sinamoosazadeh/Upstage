# APEX_GEN5 — PHASE 2 COMMON PROTOCOL (LAW FOR EVERY AGENT) — REV-2

Applies to: AGENT-01 … AGENT-06, equally, without exception. This file preserves and re-bases the binding sections of `PROMPT.md` (Chief Engineer directive) for a multi-agent chain. The same global directives are ALSO embedded verbatim as PART G inside every agent prompt (`PHASE2_PROMPT_AGENT_0N.md`; canonical copy `PHASE2_GLOBAL_DIRECTIVES.md`) — an agent may never rely on "the repo file said so" for law it skipped: the prompt itself carries the obligation. This PROTOCOL additionally governs chain mechanics (handoff format, ownership, rebase, resume). Where both speak, neither contradicts: PART G = duty, this file = procedure. Read this file IN FULL once at session start. Short on purpose.
...
- Frozen specification: `APEX_GEN5.md` (repo root). It is the single authoritative source of implementation truth. Before writing any code you MUST read, in full, the blueprint ranges assigned to you in your prompt file (plus the always-binding layers: Ch.1–2, GLOBAL IDENTITY/PIT contract L4104–4158, §9.5), and build your internal implementation map for your scope. No coding begins before your assigned specification is understood.
- Legacy repository `https://github.com/sinamoosazadeh/APEX_GEN5` (previous partial attempt, commit 1efe203, `APEX_GEN5.md` copy verified byte-identical to ours): REFERENCE MATERIAL ONLY, accessed strictly through `PHASE2_SALVAGE.md` dispositions (see P14-bis). It is never a source of truth; nothing is adopted without clause-level re-verification against the blueprint; nothing is ever pushed to it.
...
- Logical, atomic commits with clear messages in this exact shape: `[CP-n][AGENT-nn] <subsystem>: <what changed>` (e.g., `[CP-1][AGENT-01] apex/identity: canonical_json + uuid_v7 + snapshot per §9.5-5`).
...
- Before EVERY push: `git pull --rebase origin main`. If the push races (another parallel slot pushed first), rebase and push again. Conflicts: if they occur in shared `PHASE2_*` control files, resolve by KEEPING BOTH blocks (append-merge); if a conflict occurs anywhere else, it means an ownership violation — stop, do not force-resolve, log it as a CRITICAL issue in DECISION_LOG.
- NEVER: force-push, history rewrite, rebasing or reverting other agents' commits, `git clean -fdx` on untracked work you don't own, temporary files, generated garbage, unrelated modifications. Never commit `data/`, DBs, `.env`, or secrets (ADR-P2-013: `.gitignore` owned by AGENT-01).
...
## P14-bis. SALVAGE LAW (AGENT-01/AGENT-02 only; binding procedure)

1. Source: `git clone --depth 1 https://github.com/sinamoosazadeh/APEX_GEN5.git` into your scratch space (NOT into Upstage's tree, NOT via cherry-pick, NOT via submodule). Read `PHASE2_SALVAGE.md` rows assigned to you.
2. Per row: re-open the cited blueprint ranges FIRST; then diff the legacy implementation clause-by-clause; adopt into the normative tree path ONLY after conformance, with your own tests proving each adopted clause; otherwise rewrite (default on any doubt — G4). Legacy comments/paths/identifiers that do not exist in the blueprint do not survive adoption.
3. Every adopted file gets commit trailer lines: `Salvaged: <legacy path> @1efe203` and `Verified-against: APEX_GEN5.md <ranges>`; fill the row's disposition in `PHASE2_SALVAGE.md` (your CP part of the table) and mirror it into your handoff §DELIVERED (same path, plus `salvaged-conformed`/`rewritten` tag).
4. Prohibited: importing the legacy repo as history; creating `apex/candle_intelligence/`; keeping a table not in the canonical DDL; inheriting placeholders ("simplified", "TODO", `Decimal("0")  # placeholder`); trusting a legacy docstring as a claim of conformance.
5. The salvage work is INSIDE your checkpoint scope, not an extra phase; it may not displace any mandatory T-id test. If budget tightens, drop salvage and build clean (state so in the handoff with the row marked `SKIPPED(reason)`).
...
- READ IN FULL, in order: (1) your prompt file (its PART G is the whole law — you already carry it); (2) this PROTOCOL; (3) `PHASE2_CHECKPOINTS.md` (your CP section only); (4) `PHASE2_DECISION_LOG.md` (rules + open-issues + ADR table); (5) if your CP is CP-1 or CP-2: `PHASE2_SALVAGE.md` (your rows); (6) the handoff file(s) of your predecessor CP(s) named in your prompt — capped at 400 lines per section by their authors precisely so this read is cheap; (7) YOUR assigned blueprint ranges from `APEX_GEN5.md` (use the line ranges; read them in 400–800 line slices via file tools, never re-read twice — extract notes as you go). Staged roles (CP-5): read only your stage's named predecessor sections.
...
- Track your own budget: if you estimate <25% remaining while mid-CP, STOP feature work and execute P16 RESUME. Wasting budget on reading what prior agents wrote is how this project's first attempt died; it will not be repeated.
...
## P18. FINAL SELF-AUDIT before declaring your CP complete (PROMPT.md §17) + report sections (PROMPT.md §18)

Before push, audit your own work: Specification Compliance (every required component in your range implemented?); Architecture Compliance (designed structure preserved — tree paths exact, no renames, no added layers); Runtime Compliance (does YOUR surface actually execute — you ran the tests, not just wrote them); Completeness (any unfinished code remains? grep your scope for TODO/FIXME/NotImplementedError/pass-stub before declaring); Scope Compliance (no unauthorized features; Wave-Out respected).
The full-project final report (`PHASE2_FINAL_REPORT.md`, assembled by AGENT-06) keeps PROMPT.md §18 structure: Repository Report; Implementation Report (purpose/location/status/integration per subsystem); Engine Report (E01–E12 implementation/formula/validation status); Runtime Report (startup result, Telegram status, operational verification); Testing Report (executed tests, results, issues resolved); Deployment Report (environment, installation, final run command); Blueprint Compliance Declaration (no omission, no unauthorized behavior, no placeholder, no deviation). Your per-CP report feeds these sections; you write none of the others' claims.

## P19. INTERFACE-CODE-AGAINST rule (how parallel/serial slots stay compatible)

You code against interfaces in this priority: (1) a pushed, real module from a completed predecessor — import it as-is, never fork/copy it; (2) a frozen contract from the blueprint for a sibling that will exist by CP-7 (engine v4.0.0 schemas, AI.2 table) — consume it exactly as specified, including the specified degradation path when the provider is absent at your build time; (3) never invent a third. The integration audit (CP-6) reconciles (2)-against-(1); that is by design and costs you nothing.

## P20. Numbering and naming integrity

Checkpoints: CP-1…CP-6 (CP-7 exists on the board only as the owner's closure stamp — no code scope). Agent slots: AGENT-01…AGENT-06 (zero-padded), one role per checkpoint; CP-5's role runs sessions as stages §[RUNTIME] → §[TELEGRAM-OPS] → §[RESEARCH]; CP-6's role is the independent audit session (fresh chat, may reuse a previous account — ADR-P2-016). Test ids exactly as AI.10 (`T-DC-001` …) plus acceptance ids (`T_VETO`, `T_MATCH`, `T_RECONCILE`, `T_LEDGER`, `T_ADAPTER_SUBMIT`, `T_ADAPTER_DUPLICATE`, `T_ADAPTER_LOST_ACK`, `T_MONOTONE`) and fixture ids (`FIX_*` per engine, `GF_SC_01/02`). Engine ids E01…E12. File names: exactly `APEX_GEN5.md`, `PROMPT.md`, and the `PHASE2_*` set of the master plan §6. Any new identifier you introduce (fixture files, migration names, salvage tags) follows the existing scheme and is recorded in your handoff. Never rename or renumber anything you pass along.
