...
- **Push state (honest, at handoff time):** `1d06481`, `83a651c` and `55e90e7` are on the remote (verified by the `git push` ref update `1d06481..55e90e7`). `8426680`, `7957bc2` and the branch-HEAD docs commit are committed locally and **pending push**: the sandbox `GH_TOKEN` expired mid-stage (`gh auth status` → "The github.com token in GH_TOKEN is no longer valid"; `git push`/`git ls-remote` → "could not read Username … terminal prompts disabled"). This is an environment credential outage, not a branch-pin conflict — see G14. Re-push with `git push origin arena/01a0953f-upstage` once GitHub auth is restored; nothing else is outstanding.
...
**none — stage fully closed.** All three §8 batteries are green and the handoff/MATRIX/board/DECISION_LOG deliverables are written. One mechanical item is outstanding: the last three commits (`8426680`, `7957bc2` and the branch-HEAD docs commit) are committed but unpushed because the sandbox GitHub token expired (see STATUS); they need `git push origin arena/01a0953f-upstage` after auth is restored. No CP-4 code, test or fixture work remains.
