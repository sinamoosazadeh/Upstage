...
CP-14.6-FIX (2026-09-25), same branch, same PR `#24`. ISSUE-CP14-068 CLOSED by D63. ISSUE-CP14-069 CLOSED by D62. ISSUE-CP14-072 CLOSED. ADR-CP14-033 records this fix and discloses that blueprint SHOCK is defined as CRISIS, and that D62 writes the fourteen `q_thr_by_tf` rows. `APEX_GEN5.md` sha256 after this fix is `493568887acb7164c985a0310809140ef3a6b3651e15d05782708b7f73e98569`. `data/e11_preview_720.yaml` is not in this sandbox; its digest is not invented here.

Six phone commands. Run them from the repository root after the new head is on `origin/arena/01a0d4bf-upstage`.

1. `git pull --ff-only origin arena/01a0d4bf-upstage`  
   Expected: a fast-forward to the new head. If it is already current, `Already up to date.` Stop if the pull is not a fast-forward.

2. `cp data/e11_preview_720.yaml params/e11_classifier_v1.yaml`  
   Expected: the copy exists at `params/e11_classifier_v1.yaml`. Do not commit that file.

3. `sha256sum params/e11_classifier_v1.yaml`  
   Expected: one line, `<digest>  params/e11_classifier_v1.yaml`. The digest is the phone file's digest. It is not invented in this sandbox.

4. `python scripts/run_apex.py publish-quality-backfill --json` run twice.  
   Expected first run: JSON with a `written` count for the PAPER backfill. Expected second run: the same command prints `written` of `0`.

5. `python scripts/run_apex.py serve --cycles 1 --json` run twice.  
   Expected first run: a cycle JSON object. `BOOT_NOT_READY` is expected until CP-15 if new trades are not allowed. Expected second run: `cells_skipped_already_decided` greater than `0` because the first cycle wrote the close cursor.

6. `python scripts/run_apex.py status --json`  
   Expected: `artifact` is `E11_ARTIFACT` after command 2. If the classifier file is missing or unreadable, the field is the loader refusal, not a guessed name.

UNTICKED CHECKBOXES — every unticked box in `APEX_GEN5.md`:
